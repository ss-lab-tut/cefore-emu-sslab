...
def select_template(idx):
    if idx < 3:
        return TEMPLATE_ROOT / f"h{idx}"
    return TEMPLATE_ROOT / "h0"


def ensure_node_dirs(hostNum):
    rng_templates = [select_template(idx) for idx in range(hostNum)]
    for idx, template in enumerate(rng_templates):
        node_dir = f"h{idx}"
