...
**完了 (2026-07-31) — mutation testing 第1回 (`src/core/config/validator.py`)**:
mutmut 2.5.1 で 1568 mutant。raw score 58.7%→59.4% (killed 920→932、退行 0)。既存テストメソッドへの 7 パターン追加 (parametrize タプル 1 + 完全一致アサートへの狭化 6) で 12 体を kill、全件 leave-one-out (パターンを外すと生存に戻る) で因果を実証。**根本原因: `validate_config()` のエラーメッセージは返り値＝API 契約なのに、テストのアサートが `assert any("field" in e for e in errors)` の部分一致 140 箇所に対し完全一致は 11 箇所しかない。mutmut は文字列を `XX…XX` で包むので部分一致は素通しする**。完全一致への狭化は本文だけでなく prefix 組み立て (`_validate_ccninfo_options(errors, f"events[{idx}]", ...)`) まで固定するため守備範囲が広い。併せて `tests/core/test_fib.py` の `assert ... or True` (恒真＝ループ本体が無検証) を除去。残 636 生存は未分類 (うち 51 は uncovered＝カバレッジ穴)。詳細レポートは /tmp (repo 非コミット)。GitHub issue 化は PAT の権限不足 (HTTP 403) で不可。
_Avoid_: mutmut 3.x を使うこと (`record_trampoline_hit` の `assert not name.startswith("src.")` が literal `src` パッケージを拒否、かつ `--paths-to-mutate`/`--runner`/`--use-coverage`/`result-ids` が存在しない別方言)、`mutmut result-ids` の出力を正規化せず `while read` に渡すこと (全 ID が 1 行スペース区切りで来るのでループが 1 回しか回らず、部分集合判定が等号で偽陽性合格する)、`mutmut run <id>` の単体再実行を検証オラクルに使うこと (生存する mutant に `ok_killed` を返した実測あり — 検証は手動 A/B で行う)、in-place mutation を共有ツリーで走らせること (専用 detached worktree で実行する)、スコープ内テストだけの runner で得た survived を全被覆確認なしに gap として報告すること (実測で 790 件中 143 件が偽の生存者だった)
