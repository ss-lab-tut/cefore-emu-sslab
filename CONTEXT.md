...
feature/seam (= main, PR#13 マージ後) を対象に 8 subsystem 並列探索 + 候補ごと adversarial 検証を実施、21 raw 候補中 20 生存。bug 級 B1・B2 は 2026-07-09 に解消済み (下記)。Strong S1 (run_cefstatus deepening)・S2 (dead per-content label param 削除) は 2026-07-12 に実装完了、エントリ削除済み。残る優先順: (1) Strong S3〜S9、(2) Worth exploring W1〜W8 は余力で、Speculative P1〜P2 は保留。独立 housekeeping 負債は解消済み (2026-08-01 PR A fix-first): `mypy src` **0 errors**・`ruff check src tests` **0 errors** (いずれも fresh-cache 実測。stubs 追加 + mininet.* override + 実負債 30 件を type:ignore 0 件で本修正返済)。以後この 2 つは CI の zero-error gate が守る (ADR-0004)。mypy 計測は従来どおり fresh-cache (--no-incremental) 必須。
