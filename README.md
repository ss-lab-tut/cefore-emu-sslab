...
sudo ceforeemu disaster --config configs/examples/example.yaml
```
...
sudo python3 tools/autotest/run.py \
  --base-config configs/examples/example.yaml \
  --runs 5 \
  --duration 120 \
  --out out
```
...
│
├── connect.py                     # Legacy entry point (external_net)
...
- `configs/templates/h*/default-private-key` files contain sensitive cryptographic material
