...
- **新 interface 側は DONE**: `MeshBuildSpec.switch_limit` (`runtime/scenario_setup.py:133`; 2026-09-17 再測、旧記述 :124 から更新)、adapter :167 で `swhich_num=spec.switch_limit` に変換。
...
- (2026-09-17 訂正) ticket 名の「typo」は 95f175a に合わせ「legacy 綴り」と読むこと: `switch_num` が正しい綴りだという根拠は repo に無く、改名先は semantic 名 `switch_limit`。
