...
## Status (2026-09-17 訂正)

- **解消済み** (7a545f8, 2026-08-25): `_validate_flap_descriptor` (src/core/config/validator.py:747) へ抽出し、simple / cycles[idx] の双方が利用、重複 diagnostic も除去。`_validate_failure_scenarios` は現在 `validator.py:798-854` (次の def `_validate_bridges` が :855)。
- duration/interval >= 1 への tightening は S5 ではなく R8 ([17](17-r8-failure-policy.md)) の scope (95f175a, 2026-09-02 再確認)。
- 旧記述 (2026-08-23 時点): 未着手、`validator.py:747-897` に simple/cycles の重複検査が現存、R8 同梱かは着手時判断 — いずれも上記で失効。

## Resolution (2026-08-24)

- main への反映 commit は 7a545f8 (commit 日付 2026-08-25、0.2.1 へ bump)。
